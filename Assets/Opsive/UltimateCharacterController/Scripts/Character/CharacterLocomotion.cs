    ...
    public abstract class CharacterLocomotion : StateBehavior, IForceObject
    {
        ...
        private void DeflectHorizontalCollisions()
        {
            ...
            for (int i = 0; i < hitCount; ++i) {
                ...
                var hitRigidbody = m_CombinedRaycastHits[i].transform.gameObject.GetCachedParentComponent<Rigidbody>();
                var canStep = true;
                if (hitRigidbody != null) {
                    canStep = !PushRigidbody(hitRigidbody, m_PlatformMovement, m_CombinedRaycastHits[i].point, radius);
                }
                ...
                if (m_Grounded && canStep) {
                    ...
                    if (groundPoint.y <= m_MaxStepHeight + c_ColliderSpacing) {
                        ...
                        m_SlopeRay.origin = m_CombinedRaycastHits[i].point - horizontalDirection.normalized * (c_ColliderSpacing + 0.1f);
                        m_SlopeRay.direction = horizontalDirection;
                        m_CombinedRaycastHits[i].collider.Raycast(m_SlopeRay, out m_RaycastHit, (c_ColliderSpacing + 0.11f));
                        ...
                        } else if (OverlapCount(activeCollider, horizontalDirection + m_PlatformMovement + m_Up * (m_MaxStepHeight - c_ColliderSpacing)) == 0) {
                    ...
                    for (int i = 0; i < hitCount; ++i) {
                        ...
                        if (m_Grounded && Vector3.Angle(m_Up, m_GroundRaycastHit.normal) <= m_SlopeLimit + 0.3f) {
                            ...
                            if (groundPoint.y <= m_MaxStepHeight + c_ColliderSpacing) {
                                // A CapsuleCast/SphereCast normal isn't always the true normal: http://answers.unity3d.com/questions/50825/raycasthitnormal-what-does-it-really-return.html.
                                // Cast a regular raycast in order to determine the true normal.
                                m_SlopeRay.origin = m_CombinedRaycastHits[i].point - hitMoveDirection.normalized * (c_ColliderSpacing + 0.1f);
                                m_SlopeRay.direction = hitMoveDirection;
                                m_CombinedRaycastHits[i].collider.Raycast(m_SlopeRay, out m_RaycastHit, (c_ColliderSpacing + 0.11f));
                                var slope = Vector3.Angle(m_Up, m_RaycastHit.normal);
                                if (slope <= m_SlopeLimit + 0.3f) {
                                    continue;
                                ...
                                } else if (OverlapCount(activeCollider, hitMoveDirection + m_PlatformMovement + m_Up * (m_MaxStepHeight - c_ColliderSpacing)) == 0) {
                                    // Step over the object if there are no objects in the way.
                                    continue;
                                }
                            }
                        ...
                        var hitMoveDistance = Mathf.Min(m_CombinedRaycastHits[i].distance - moveDistanceContribution - c_ColliderSpacing, hitMoveDirection.magnitude);
                        if (hitMoveDistance < 0.001f) {
                            hitMoveDistance = 0;
                        }
        ...
        private bool PushRigidbody(Rigidbody targetRigidbody, Vector3 moveDirection, Vector3 point, float radius)
        {
            ...
            targetRigidbody.AddForceAtPosition((moveDirection / Time.fixedDeltaTime) * (m_Mass / targetRigidbody.mass) * 0.1f, point, ForceMode.VelocityChange);
