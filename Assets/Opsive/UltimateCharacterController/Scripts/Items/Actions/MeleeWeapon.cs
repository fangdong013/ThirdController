...
namespace Opsive.UltimateCharacterController.Items.Actions
{
    ...
    public class MeleeWeapon : UsableItem
    {
        ...
        /// <param name="startAbility">Is the ability starting?</param>
        /// <returns>True if the item can be used.</returns>
        public override bool CanUseItem(ItemAbility itemAbility, bool startAbility)
        {
            ...
            if (startAbility && m_Attacking) {
                if (!m_Used) {
                    // The weapon needs to be used before it can be used again.
                    return false;
                } else if (m_AllowAttackCombos) {
                    // The weapon can be used if it is currently attacking, has been used, and allows attack combos. This allows the next state to be played
                    // while the current state is active (thus chaining the attacks).
                    return true;
                }
            }

            if (!base.CanUseItem(itemAbility, startAbility)) {
                return false;
            }
        ...
        private bool HitCollider(MeleeHitbox hitbox, Collider other)
        {
            ...
            if (hitCount > 0) {
                ...
                for (int i = 0; i < hitCount; ++i) {
                    ...
                    var hitGameObject = m_CollisionsHit[i].transform.gameObject;
