    ...
    public static class CharacterBuilder
    {
        ...
        public static void AddEssentials(GameObject character, bool addPerspectiveMonitor, bool aiAgent)
        {
            ...
            if (colliderIdentifier == null) {
                ...
                var collider = new GameObject("CapsuleCollider");
        ...
        public static void AddFootEffects(GameObject character)
        {
            if (character.GetComponent<CharacterFootEffects>() == null) {
                ...
                // Add the humanoid feet.
                var animator = character.GetComponent<Animator>();
                if (animator != null) {
                    var head = animator.GetBoneTransform(HumanBodyBones.Head);
                    if (head != null) {
                        footEffects.Feet = new CharacterFootEffects.Foot[2];
                        // Try to use the toes if the bones have been mapped.
                        var toe = animator.GetBoneTransform(HumanBodyBones.LeftToes);
                        if (toe != null) {
                            footEffects.Feet[0].Object = toe;
                        } else {
                            footEffects.Feet[0].Object = animator.GetBoneTransform(HumanBodyBones.LeftFoot);
                        }
                        if (footEffects.Feet[0].Object != null && footEffects.Feet[0].Object.gameObject.GetComponent<AudioSource>() == null) {
                            var audioSource = footEffects.Feet[0].Object.gameObject.AddComponent<AudioSource>();
                        ...
                        if (toe != null) {
                            footEffects.Feet[1].Object = toe;
                        } else {
                            footEffects.Feet[1].Object = animator.GetBoneTransform(HumanBodyBones.RightFoot);
                        }
                        if (footEffects.Feet[1].Object != null && footEffects.Feet[1].Object.gameObject.GetComponent<AudioSource>() == null) {
                            var audioSource = footEffects.Feet[1].Object.gameObject.AddComponent<AudioSource>();
                            audioSource.volume = 0.4f;
                            audioSource.playOnAwake = false;
                            audioSource.spatialBlend = 1;
                            audioSource.maxDistance = 20;
                        }
                        footEffects.Feet[1].FlippedFootprint = true;
                    }
                }
            }
        }
